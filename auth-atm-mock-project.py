...
def withdrawal_operation(account_number,balance):
    print("withdrawal")
    # get current balance
    ...
    users = os.listdir(user.user_db_path)
    try:
        for user in users:
            print(user_list)
        ...
        try:
            users = open(user.user_db_path + str(account_number) + ".txt", "a"
            except ("Can not update file")
...
def deposit_operation(account_number,amount):
